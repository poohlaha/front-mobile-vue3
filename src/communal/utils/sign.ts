...
const Signature = {
  ...
  sign: (headers: any = {}) => {
    ...
    headers["sophia_superficial"] = CryptoJS.HmacSHA1(
      timestamp + nonce + echoStr,
      SYSTEM.SIGNATURE.SIGNATURE_KEY,
    );
  },
  ...
  encrypt: (
    data: any,
    publicKey = SYSTEM.SIGNATURE.PUBLIC_KEY,
    iv = SYSTEM.SIGNATURE.CBCIV,
  ) => {
...
const setHeaders = (config: any = {}) => {
  ...
  if (SYSTEM.NEED_TOKEN) {
    headers[SYSTEM.TOKEN_NAME] =
      Utils.getLocal(`${SYSTEM.OPEN_ID}_${SYSTEM.LOCAL_TOKEN}`) || "";
  }
