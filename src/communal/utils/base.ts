...
const PAGE_JUMP = {
  ...
  setUserInfo: (userInfo: any = {}) => {
    ...
    Utils.removeLocal(`${SYSTEM.OPEN_ID}_${SYSTEM.USER_TOKEN}`);
    Utils.setLocal(
      `${SYSTEM.OPEN_ID}_${SYSTEM.USER_TOKEN}`,
...
const ADDRESS = {
  ...
  getUrlString: (url: string) => {
    ...
    const getQueryParams = (url: string = "") => {
      ...
      const query =
        t === "?" ? url.substring(1, url.length).split("&") : url.split("&");
...
const USER = {
   ...
   */
  getUserInfo: () =>
    Utils.getLocal(`${SYSTEM.OPEN_ID}_${SYSTEM.USER_TOKEN}`) || {},
  ...
  setUserInfo: (userInfo: { [K: string]: any } = {}) => {
    ...
    Utils.setLocal(
      `${SYSTEM.OPEN_ID}_${SYSTEM.USER_TOKEN}`,
      JSON.stringify({ phoneInfo, accountInfo }),
    );
