...
class Updater {
  ...
  readConfigFile() {
    if (!fsExtra.pathExistsSync(this.configFilePath)) {
      console.log(
        LOGGER_PREFIX,
        chalk.magenta("根目录下不存在 `.updaterc` 文件 !"),
      );
      return;
    }
  ...
  analysisConfig() {
    ...
    // 过滤不存在的文件
    let dests = this.dests.filter((dest) =>
      fsExtra.pathExistsSync(path.resolve(this.appRootDir, dest)),
    );
    let ignores = this.ignores.filter((ignore) =>
      fsExtra.pathExistsSync(path.resolve(this.appRootDir, ignore)),
    );
    this.dests = dests;
    this.ignores = ignores;
    ...
    this.ignores.forEach((ignore) => {
      if (BaleUtils.Paths.isDir(ignore)) {
        // 目录
        ignoreFiles = ignoreFiles.concat(
          BaleUtils.Paths.getFileList(ignore) || [],
        );
      } else {
  ...
  handUpdate(projectDir) {
    ...
    if (fileList.length > 0) {
      ...
      // 清空原来目录
      BaleUtils.Paths.cleanDir(
        path.join(this.updateDir, this.updateProjectName),
        false,
      );
      // 拷贝到原始目录
      fsExtra.copySync(
  ...
  async updater(flag = false) {
    ...
    try {
      ...
      // 更新 package.json
      const hasWrite = this.updatePackageJson(
  ...
  updatePackageJson(destPackageJson) {
    if (!fsExtra.pathExistsSync(destPackageJson)) {
      console.log(
        LOGGER_PREFIX,
        chalk.magenta("框架中 package.json 不存在, 无法更新 !"),
      );
      return false;
    }
    ...
    if (!fsExtra.pathExistsSync(appPackageJson)) {
      console.log(
        LOGGER_PREFIX,
        chalk.magenta("项目中 package.json 不存在, 无法更新 !"),
      );
      return false;
    }
    ...
    let hasDependenciesWrite = rewriteDependencies(
      descPackage.dependencies,
    ...
    let configFlag =
      _.isEqual(appPackageHusky, descPackage["husky"]) &&
      _.isEqual(appPackageLintStaged, descPackage["lint-staged"]);
    ...
    if (hasWrite) {
      ...
      fsExtra.writeFileSync(
        path.join(this.appRootDir, "package.json"),
